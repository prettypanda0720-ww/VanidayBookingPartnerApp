...
import { getFormattedDate, getCurrentMonth, getCurrentDay, getCurrentDate } from '../utils';
...
const getDateTextStyles = (numberOfDays) => {
  const fontSize = numberOfDays === 7 ? 16 : 18;
  return {
    fontSize,
  };
};
...
const getDayTextStyles = (numberOfDays) => {
  const fontSize = numberOfDays === 7 ? 10 : 12;
  return {
    fontSize,
  };
};
...
const Column = ({
  ...
  return (
    <View style={styles.column}>
      <Text style={[ { color: textColor}, getDateTextStyles(numberOfDays)]}>
        {getCurrentDate(column)}
      </Text>
      <Text style={[ { color: textColor }, getDayTextStyles(numberOfDays)]}>
        {getCurrentDay(column)}
      </Text>
    </View>
  );
};
